...
UI jest po hiszpansku, znakami Unicode. Numeric &# w etykiecie psuje kolor.
...
def main() -> int:
    ...
    numeric_entities = raw.count("&amp;#") + raw.count("&#")
    if numeric_entities:
        add("encje &# w UI (hash = kolor)", str(numeric_entities))
    spanish_letters = sorted({c for c in raw if c in "áéíóúñüÁÉÍÓÚÑÜ¡¿"})
    if spanish_letters:
        info["litery ES w pliku"] = "".join(spanish_letters)

    counts = {"puste": 0, "klawisze": 0, "przetlumaczone": 0, "zostawione": 0}
    ...
    for identifier, text in entries.items():
        ...
        if "[EN]" in text or "[ES]" in text:
            add("zostal blok EN/ES", identifier)
        ...
        if original and normalize(original) == normalize(text):
            counts["zostawione"] += 1
            add("zostawione po francusku", identifier)
            continue
        counts["przetlumaczone"] += 1
        if original and brace_tokens(original) != brace_tokens(text):
            add("rozjechane {placeholdery}", identifier)
        if original and engine_vars(original) != engine_vars(text):
            problems_list = describe_problems(original, text)
            add(
                "rozjechane zmienne silnika",
                f"{identifier}: {problems_list[0] if problems_list else 'mismatch'}",
            )
        if original and looks_untranslated(original, text):
            add("zdanie zostawione w jezyku zrodla", identifier)
        plain_es = html.unescape(text)
    ...
    print(
        f"Stringi: {counts['przetlumaczone']} po hiszpansku, "
        f"{counts['zostawione']} bez zmian, "
