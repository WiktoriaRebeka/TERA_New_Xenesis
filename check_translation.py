...
def main() -> int:
    ...
    if source_path.is_file():
        ...
        for identifier, (_tooltip, _name) in source_entries.items():
            if identifier not in entries:
                continue
            english_name = en_item_string(identifier or "")
            if english_name is not None and entries[identifier][1] != english_name:
                add("nazwa przedmiotu nie jest z EN", f'{identifier}: "{entries[identifier][1]}"')
    else:
        source_entries = {}
    ...
    for identifier, (tooltip, _) in entries.items():
        ...
        if "[EN]" in tooltip or "[ES]" in tooltip or CREDIT in tooltip:
        ...
        english = source_entries.get(identifier, ("", ""))[0]
        spanish = tooltip
