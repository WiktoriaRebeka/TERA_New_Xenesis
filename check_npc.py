"""
Kontrola jakosci StrSheet_Npc (sam hiszpanski).
"""
    ...
    for path in files:
        ...
        for identifier, value in entries.items():
            ...
            if "[EN]" in value or "[ES]" in value:
                print(f"{path.name} id={identifier}: zostal blok EN/ES")
                problems += 1
                continue
            if original and value == original:
                print(f"{path.name} id={identifier}: zostawione po francusku")
                problems += 1
                continue
            issues = describe_problems(original, value) if original else []
            if original and looks_untranslated(original, value):
