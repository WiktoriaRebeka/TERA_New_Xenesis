"""
Translate TERA StrSheet_Item toolTip attributes from French to Latin American Spanish.
string= is restored from the English dump by id. Output is Spanish only.
...
KEEP_ENGLISH = (
    "MP", "HP", "Feedstock", "Battle Solution", "Prime Battle Solution",
    "Spellbind", "Alkahest", "Noctenium", "Everful Nostrum", "Etching",
    "Crystal", "Emerald", "Diamond", "Talent", "Focus", "Rage", "Runemarks",
    "Bravery Potion", "emote",
)
...
FORBIDDEN_ES = (
    r"\bPH\b", r"\bPV\b", r"\bPM\b", r"\baguante\b", r"\btimes\b",
...
SYSTEM_PROMPT = """
You are an expert video game localizer. Translate TERA MMORPG item tooltips from French to Latin American Spanish.

The source text is French. Write Spanish only. The French client uses PV for hit points and PM for mana: write HP and MP. Class and race names stay English even when the French source translated them: Warrior, Lancer, Slayer, Berserker, Sorcerer, Archer, Priest, Mystic, Reaper, Gunner, Brawler, Ninja, Valkyrie, Human, Castanic, Aman, Popori, Baraka, High Elf, Elin.
...
Do not include the French or English source text in your output.
Translate EVERY sentence. A long tooltip may contain many sentences separated by __TAGn__ line breaks - each one must be rendered in Spanish. Never copy a French or English sentence unchanged into your output, not even a technical one about HP, MP, cooldowns, percentages or durations.
...
OUTPUT_TEMPLATE = "{spanish}"

# French UI/class labels rewritten to English before Gemini. Filled by translate_ui.
EXACT_REWRITE: dict[str, str] = {}
EN_ITEM_DIR = Path(
...
def en_item_string(item_id: str) -> str | None:
    ...
    if _ITEM_STRINGS is None:
        ...
        _ITEM_STRINGS = found
    return _ITEM_STRINGS.get(item_id)


LOCK_ITEM_STRINGS = False


def apply_en_item_string(line: str) -> str:
    if not LOCK_ITEM_STRINGS or 'string="' not in line:
        return line
    id_match = re.search(r'\bid="(\d+)"', line)
    if not id_match:
        return line
    english = en_item_string(id_match.group(1))
    if english is None:
        return line
    return re.sub(
        r'\bstring="[^"]*"',
        'string="' + xml_attr_escape(english) + '"',
        line,
        count=1,
    )
...
FRENCH_STOPWORDS = {
    "le", "la", "les", "de", "des", "du", "un", "une", "et", "vous", "est",
    "dans", "pour", "avec", "sur", "que", "qui", "pas", "au", "aux", "ce",
    "cette", "votre", "vos", "par", "plus", "son", "sa", "ses",
}


def _copied_sentence(words: list[str]) -> bool:
    if len(words) < 4:
        return False
    english = sum(1 for w in words if w in ENGLISH_STOPWORDS and w not in _NAME_STOPWORDS)
    french = sum(1 for w in words if w in FRENCH_STOPWORDS)
    return english >= 2 or french >= 2
...
def breaks_glossary(original: str, spanish_escaped: str) -> bool:
    ...
    for term in KEEP_ENGLISH:
        ...
        if re.search(pattern, en, re.I) and not re.search(pattern, es, re.I):
            return True
    for src_term, dst_term in (("PV", "HP"), ("PM", "MP")):
        if re.search(rf"\b{src_term}\b", en) and not re.search(rf"\b{dst_term}\b", es):
            return True
...
def describe_problems(original: str, candidate: str) -> list[str]:
    ...
    for segment in set(_segments(en_text)) & set(_segments(es_text)):
        words = _plain_words(segment)
        if _copied_sentence(words):
            problems.append(f"this sentence is still French or English: {segment[:120]}")
    for src_term, dst_term in (("PV", "HP"), ("PM", "MP")):
        if re.search(rf"\b{src_term}\b", en_text) and not re.search(rf"\b{dst_term}\b", es_text):
            problems.append(f"the French stat {src_term} must be written {dst_term}")
    for term in KEEP_ENGLISH:
        pattern = r"\b" + re.escape(term)
...
def rewrite_lines(
    ...
    for line in lines:
        match = TOOLTIP_RE.search(line)
        if not match:
            output_lines.append(apply_en_item_string(line))
            stats["no_tooltip"] += 1
            continue
        ...
        exact = EXACT_REWRITE.get(html.unescape(original).strip())
        if exact is not None:
            start, end = match.span(1)
            output_lines.append(apply_en_item_string(line[:start] + xml_attr_escape(exact) + line[end:]))
            stats["applied"] += 1
            continue
        ...
        output_lines.append(apply_en_item_string(line[:start] + new_value + line[end:]))
        stats["applied"] += 1
