"""Shared Spanish-only string= rewriter for Tutorial, Dungeon and SystemMessage sheets."""
...
# Optional: replace a French string with its English proper name.
REPLACE_EXACT = None
    ...
    for line in lines:
        for match in ATTR_RE.finditer(line):
            ...
            if original == "" or original in seen:
                continue
            if REPLACE_EXACT is not None and REPLACE_EXACT(original) is not None:
                continue
    ...
    for line in lines:
        ...
        for match in reversed(matches):
            ...
            if original == "":
                stats["empty"] += 1
                continue
            if REPLACE_EXACT is not None:
                replacement = REPLACE_EXACT(original)
...
def list_sheet_files(root: Path, glob_pat: str, start: int, count: int) -> list[Path]:
    files = sorted(
        path for path in root.glob(glob_pat) if path.name != "StrSheet_Dungeon-00000.xml"
    )
    if start < 0:
        start = 0
    files = files[start:]
